...
class GPTAPI(BaseAPILLM):
                ...
                if chunk:
                    ...
                    try:
                        ...
                        if choice['finish_reason'] == 'stop':
                            return
                        yield choice['delta'].get('content') or ''
    ...
    def generate_request_data(self, model_type, messages, gen_params, json_mode=False):
        ...
        elif model_type.lower().startswith('internlm') or model_type.lower().startswith('intern-') or model_type.lower().startswith('intern/'):
            data = {'model': model_type, 'messages': messages, 'n': 1, **gen_params}
            if json_mode:
                data['response_format'] = {'type': 'json_object'}
        else:
